...
/// </para>
/// <para>
/// Blinky's list, which this came from, is missing <c>FF</c>: SET MANAGEMENT
/// KEY carries the new key in the clear, and the comment on <c>DB</c> credits
/// it with covering that — but <c>DB</c> only writes the copy into PRINTED.
/// A failed transmit of <c>FF</c> would have logged the management key of the
/// card in hex.
/// </para>
/// </remarks>
public static class ApduRedaction
{
    ...
    [
        ...
        0xDB, // PUT DATA - a new management key on its way to PRINTED
        0xFF, // SET MANAGEMENT KEY - the new key itself, in the clear
    ];
