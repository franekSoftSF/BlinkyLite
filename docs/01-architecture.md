...
| D-22 | Reset PIV (kasowanie karty) **tylko w `tools/BlinkyLite.CardLab`**, nigdy w kliencie ani w module | token testowy musi wracać do stanu fabrycznego między przebiegami, a odsyłanie po to do innego narzędzia kończy się kartą w połowie sprowizjonowaną; produkt dalej tylko wydaje i weryfikuje, a `RESET` wymaga zablokowania PIN-u i PUK-u, więc nie da się go wykonać przez przypadek |
| D-21 | **Profile na serwerze**, konfigurowane odgórnie przez admina; klient pobiera je przez `GET /api/profiles` i wysyła nazwę profilu, nigdy nazwę szablonu | jedna zmiana zamiast obchodu po stacjach; do wiersza wydania szablon wpisuje serwer ze swojej konfiguracji, więc zapis jest prawdziwy nawet przy podmienionej stacji — tego, co stacja wyśle do CA, pilnuje CA |
