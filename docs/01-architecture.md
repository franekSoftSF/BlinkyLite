...
| D-21 | Szablon ADCS wybiera operator z listy **profili** w `appsettings.json` serwera; profili może być kilka | kart jest więcej niż jeden rodzaj, a nazwa szablonu ma być zapisana w jednym miejscu; wydanie kopiuje profil i szablon do swojego wiersza, żeby zmiana konfiguracji nie przepisała historii |
