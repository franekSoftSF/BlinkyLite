...
| 0055 | nginx w Dockerze | `docker compose up` stawia nginx przed serwerem: aplikacja web statycznie, `/api` do serwera; TLS od klienta do nginx i od nginx do serwera na sieci wewnętrznej; publicznie tylko nginx; Negotiate przechodzi przez proxy bez zmian (D-26) |
